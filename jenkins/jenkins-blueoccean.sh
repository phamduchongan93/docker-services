...
# Description: Deploy jenkins blueoccean. Keep in mind that the blueoocean will use two container. One contains the docker engine and other contains the blueoccean container.
...
checkContainer () {
  ...
  docker ps | grep "$container_name"; ec="$?"
  [ $ec -eq 0 ] && exit 0
}
...
help () {
  ...
  echo '  -i,--interactive   launch program in interactive mode ' 
